# Shared-Editor

### Link alla documentazione

- [Overview del progetto](docs/Progetto.md)

- [Class Diagrams](docs/ClassDiagrams.md)

- [Comunicazioni tra client e server](docs/ClientServerCommunication.md)

- [Esempio di processamento di un messaggio](docs/esempioProcessamentoMsg.md)

- [GUI](docs/GUI.md)

- [Algoritmo CRDT](docs/crdt.md)

- [Gestione del file system](docs/fileSystem.md)

- [Esempio creazione di un cursore personalizzato](docs/customCursor.md)

- [Compilazione del progetto con CMake](docs/cmake.md)

